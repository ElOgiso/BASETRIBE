...
export async function fetchFidFromWallet(walletAddress: string): Promise<string | null> {
  try {
    ...
    if (!CONFIG.NEYNAR_API_KEY || CONFIG.NEYNAR_API_KEY.includes('your_key_here')) {
    ...
    if (data[walletAddress]?.length > 0) {
      const fid = data[walletAddress][0].fid.toString();
      console.log('✅ FID found:', fid);
      return fid;
    }

    console.log('ℹ️ No FID found for wallet:', walletAddress);
    return null;
  } catch (error) {
    if (error instanceof Error) {
      if (error.name === 'AbortError') {
        console.error('❌ Request timeout fetching FID from wallet');
      } else {
        console.error('❌ Error fetching FID from wallet:', error.message);
      }
    } else {
